...
import LandingPage from '@/components/risk/LandingPage';
import GameSetup from '@/components/risk/GameSetup';
import GameBoard from '@/components/risk/GameBoard';
import RiskGameOver from '@/components/risk/RiskGameOver';
...
function useHostHeartbeat() {
  const { gameMode, roomCode } = useRiskStore();

  useEffect(() => {
    if (gameMode !== 'diwaniya' || !roomCode) return;

    const sendHeartbeat = () => {
      fetch(`/api/risk-room/${roomCode}/heartbeat`, {
        method: 'POST',
      }).catch(() => {});
    };
  ...
  useEffect(() => {
    ...
    const handleBeforeUnload = () => {
      ...
      navigator.sendBeacon(
        `${baseUrl}/api/risk-room/${roomCode}`,
        JSON.stringify({ _method: 'DELETE' })
      );
      try {
        fetch(`/api/risk-room/${roomCode}`, { method: 'DELETE' });
      } catch { /* ignore */ }
    };
...
function GameTopBar() {
  ...
  const { phase, resetGame, gameMode, roomCode } = useRiskStore();
  ...
  const copyLink = () => {
    if (!roomCode) return;
    const link = `${window.location.origin}/join/risk/${roomCode}`;
  ...
  return (
    <>
      <div className="sticky top-0 z-50 border-b border-slate-800/50 bg-slate-950/90 backdrop-blur-sm">
        <div className="max-w-md mx-auto flex items-center justify-between px-3 py-1.5">
          <div className="flex items-center gap-1">
            ...
            {gameMode === 'diwaniya' && roomCode && (
              <motion.button
                ...
                className="flex items-center gap-1 text-[10px] sm:text-xs text-slate-400 hover:text-violet-400 transition-colors px-2 py-1 rounded-lg hover:bg-violet-950/30 cursor-pointer"
              >
          ...
          {gameMode === 'diwaniya' && !isGameOver && (
            <div className="flex items-center gap-1 bg-violet-950/40 border border-violet-500/30 rounded-lg px-2 py-1">
              <span className="text-xs">👁️</span>
              <span className="text-[10px] sm:text-xs font-bold text-violet-300">ديوانية</span>
            </div>
          )}

          {isGameOver && (
            <Badge variant="outline" className="text-[10px] sm:text-xs px-2 py-0.5 border-violet-500/50 text-violet-300">
              🏁 انتهت اللعبة
            </Badge>
          )}
      ...
      {gameMode === 'diwaniya' && roomCode && (
        <div className="bg-gradient-to-l from-violet-900/50 to-emerald-900/50 border-b border-violet-500/30 py-2 px-4">
          <div className="max-w-md mx-auto">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[10px] sm:text-xs text-violet-300">📺 شارك هذا الكود مع المشاهدين:</p>
                <p className="text-xl sm:text-2xl font-mono font-bold text-white tracking-widest">{roomCode}</p>
              </div>
              <button
                onClick={copyCode}
                className="flex items-center gap-1.5 text-xs bg-violet-800/50 text-violet-200 px-3 py-1.5 rounded-lg hover:bg-violet-700/50 transition-colors cursor-pointer"
              >
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => setShowViewers(false)} className="text-slate-500 hover:text-slate-300 text-lg cursor-pointer">✕</button>
              </div>
              ...
              {spectators.length === 0 ? (
                ...
                <div className="space-y-2 max-h-[40vh] overflow-y-auto risk-scrollbar">
                  {spectators.map((spec, idx) => (
                    <div key={spec.id} className="flex items-center gap-3 bg-slate-800/50 border border-slate-700/30 rounded-xl px-3 py-2.5">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-slate-500">انضم منذ {Math.floor((Date.now() - spec.joinedAt) / 60000)} دقيقة</p>
                      </div>
              ...
              <div className="mt-4 text-center">
                <p className="text-[10px] text-slate-600">إجمالي المشاهدين: {spectators.length}</p>
              </div>
            </motion.div>
          </motion.div>
        )}
            ...
            >
              <div className="text-center">
                ...
                <p className="text-sm text-slate-400 mb-6">
                  {gameMode === 'diwaniya' ? 'سيتم إنهاء الجلسة وسيخرج جميع المشاهدين.' : 'سيتم إعادة تعيين اللعبة بالكامل.'}
                </p>
...
export default function RiskPage() {
  ...
  const handleJoinSpectator = useCallback((code: string, name: string) => {
    window.location.href = `/join/risk/${code}?name=${encodeURIComponent(name)}`;
  }, []);

  if (!mounted) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center risk-bg">
  ...
  // PLAYING / GAME OVER
  return (
    <div className="flex flex-col min-h-screen risk-bg">
      <GameTopBar />
      <main className="flex-1">
